"""
Optimized Dataset Loader for ElevenLabs Deepfake Dataset
CPU-friendly + Stable + Cached
"""
...
class ElevenLabsDataset(Dataset): 
    def __init__(self, data_root, max_len=32000, sample_rate=16000,
        ...
        print(f"   Real: {sum(1 for l in self.labels if l == 1)}")
        print(f"   Fake: {sum(1 for l in self.labels if l == 0)}")
    ...
    def _extract_phase(self, audio):
        ...
        if phase_features.shape[0] < 128:
            phase_features = np.pad(
                phase_features,
                ((0, 128 - phase_features.shape[0]), (0, 0)),
                'constant'
            )
        else:
        ...
        if phase_features.shape[1] < 128:
            phase_features = np.pad(
                phase_features,
                ((0, 0), (0, 128 - phase_features.shape[1])),
                'constant'
            )
        else:
            phase_features = phase_features[:, :128]
...
def get_elevenlabs_loaders(data_root,
                           batch_size=8,
                           val_split=0.2,
                           augment=True,
    ...
    # Full dataset
    full_dataset = ElevenLabsDataset(
        data_root=data_root,
        augment=augment,
        cache_dir=cache_dir
    )
